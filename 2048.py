...
class Game2048(Frame):
    ...
    def key_down(self, event):
        ...
        if key in self.commands:
            self.matrix, changed = self.commands[repr(event.cha)](self.matrix)
            if changed:
                Logics_2048.add_new_2(self.matrix)
                self.update_grid_cells()
                changed = False
                if Logics_2048.get_current_state(self.matrix) == "WON":
                    self.grid_cells[1][1].configure(
                        text="You" , bg=c.BACKGROUND_COLOR_EMPTY)
                    self.grid_cells[1][2].configure(
                        text="Win!" , bg=c.BACKGROUND_COLOR_EMPTY)
                if Logics_2048.get_current_state(self.matrix) == "LOST":
                    self.grid_cells[1][1].configure(
                        text="You" , bg=c.BACKGROUND_COLOR_EMPTY)
                    self.grid_cells[1][2].configure(
                        text="Lose!" , bg=c.BACKGROUND_COLOR_EMPTY)
gamegrid = Game2048()
